...
#  vocab_size : maximum number of word in yout dictionary
    ...
    def tokenize(self):
        ...
        for key in self.data:
            ...
            for sentence in texts:
                ...
                f.write('\n')
        f.close()
        sentences = word2vec.LineSentence("txt")

        w2v = word2vec.Word2Vec(sentences, size=256)
        w2v.save('w2v.model')
        t = Tokenizer()
        for key in self.data:
            t.fit_on_texts(self.data[key][0])
        self.tokenizer = t
        '''#保存模型，供日後使用
    ...
    def load_w2v(self, path):
        print('Load w2v from %s' % path)
        self.word2vec = word2vec.Word2Vec.load("path")
...
#  maxlen : max length after padding
    ...
    def to_bow(self):
        for key in self.data:
            ...
            self.data[key][0] = self.tokenizer.texts_to_matrix(
                self.data[key][0], mode='count')
